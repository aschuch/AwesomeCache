...
public class Cache<T: NSCoding> {
    ...
    public func allObjects(includeExpired: Bool = false) -> [T] {
        var objects = [T]()
    ...
    public func setObject(_ object: T, forKey key: String, expires: CacheExpiry = .never) {
    ...
    private func expiryDateForCacheExpiry(_ expiry: CacheExpiry) -> Date {
        switch expiry {
        case .never:
